...
const PatientsTable = ({ dates }) => {
  ...
  const addWaitingRoom = async (id, type) => {
        ...
        customBodyRender: (value: any, tableMeta: any) => {
          ...
          return (
            <Box className="w-max">
              <Box
                ref={(el) => {
                  if (el) buttonRefs.current[patientId] = el;
                }}
              ...
              >
                <Tooltip title="Ajouter le patient à la salle d'attente">
                  <IconButton
                    className="btn-patient-waiting !text-gray-600 hover:!text-blue-700 cursor-pointer"
                    onClick={(e) => {
                      e.stopPropagation();
                      setAnchorEl(buttonRefs.current[patientId]);
                ...
                <Popover
                  open={Boolean(anchorEl) && selectedPatientId === patientId}
                  anchorEl={anchorEl}
                  onClose={() => {
                    setAnchorEl(null);
                ...
                >
                  <List dense>
                    {["Consultation", "Suivi", "Urgence"].map((type) => (
                      <ListItemButton
                        key={type}
                        onClick={(e) => {
                          e.stopPropagation();
                          addWaitingRoom(patientId, type);
                          setAnchorEl(null);
                          setSelectedPatientId(null);
                        }}
                      >
              ...
              {tableMeta.tableData?.[tableMeta.rowIndex]?.calculation_id && (
                <Tooltip title="Voir calculateur de grossesse">
                  <IconButton className="btn-patient-birth !text-gray-600 hover:!text-blue-700 cursor-pointer">
        ...
        <DataTable
          ...
          options={{
            ...
            onRowClick: (s: any, _m: any, e: any) => {
              ...
              if (e.target.closest(".btn-patient-birth")) {
                navigate(`/ajouter-calculateur?id=${s[9]}&see=true`);
                return;
              }
